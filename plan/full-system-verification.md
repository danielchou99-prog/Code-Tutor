...
- 文件狀態：2026-09-03 自動檢查與暫時 IP 部署完成，等待真實帳號手動驗收
- 建立日期：2026-08-09
...
- [x] 7. 提交並推送已通過驗證的版本，再唯讀確認 VPS 版本後部署。
  - 簡易說明：先確定 GitHub 與 VPS 的版本，再重建與重啟，避免再次看到舊畫面。
- [x] 8. 由暫時 IP 驗證 Dev Compass 首頁、題目列表、題目詳情、管理員題目系統與 API 健康狀態。
...
- 部署狀態：程式 commit `80aac1f` 已推送並部署；VPS Frontend、API、Judge Worker、Nginx 與 Docker 全部 enabled／active，內部 health、外部 HTTP、C++ 執行、桌面與手機題目畫面均通過。
- 管理員題目系統在未登入狀態正確顯示「受限」；登入後的新增、修改、空白欄位切換 Chrome 標籤與 Groq 真實請求仍保留給使用者手動驗收。
