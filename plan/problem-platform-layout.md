...
- [x] 18. 修正 AI Tutor 浮動視窗的垂直位置。
  - 簡易說明：面板最高只能到題目工具列下方，使關閉鍵不會靠近或蓋住上方版面控制。
- [x] 19. 所有 Compiler 預設顯示 Input。
  - 簡易說明：進入 Project 或題目時先準備輸入；Text 模式按 Run 後自動切換 Output，Interactive Console 則保留互動輸入畫面。
- [x] 20. 將測資群組改成明確的得分條件。
  - 簡易說明：不再只寫 Basic／Boundary，而是顯示「此群組限制條件、測資數與占分」。
- [x] 21. 移除 `Submit only` 並讓 Sample 數量依題目設定。
  - 簡易說明：右側只保留 Judge Result；各題可有一組、兩組或更多範例，不強迫固定數量。
- [x] 22. 重新整理後恢復原畫面。
  - 簡易說明：記住 Home／File／Problems／Settings，以及目前開啟的題目；重新整理後恢復相同位置。
- [x] 23. 在題目詳情再次點 Problems 時回到列表。
  - 簡易說明：上方 Problems 是題庫首頁入口，因此在解題頁點擊後要離開題目並顯示題目列表。
- [x] 24. 先製作資料夾式題目列表設計圖，不套用正式頁面。
  - 簡易說明：依照圖一先提供深色格狀 mockup；每個資料夾代表一道題目，資料夾下方以類似檔名的位置顯示該題標籤。等使用者核可後才修改正式 Problem List。

## 本輪執行結果

- [x] AI Tutor 關閉鍵固定在題目工具列下方，短畫面也不會蓋住上方控制。
- [x] File 與 Problems 的編譯器起始頁籤為 Input；Text 模式 Run 後切至 Output。
- [x] 題目敘述與 Judge Result 都顯示各組限制、測資數與配分。
- [x] 移除 `Submit only`，範例數量改由每道題目的資料決定。
- [x] 重新整理可恢復目前頁面與題目；解題頁點 Problems 可回題目列表。
- [x] 已輸出 `plan/mockups/problem-library-folder-grid.png`，正式列表等待使用者核可。
