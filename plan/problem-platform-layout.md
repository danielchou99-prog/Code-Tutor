...
- [x] 25. 依核可結果正式套用題目格狀列表。
  - 簡易說明：保留設計稿的資訊排列，但改用一般題目卡片，不使用資料夾圖案；桌面寬度足夠時一排至少顯示 5 題。
- [x] 26. 完整處理解題頁的上方導覽。
  - 簡易說明：在題目詳情點 Home、File、Problems、Settings 等項目，都要離開目前題目並前往指定頁面；點 Problems 顯示題目列表。
- [x] 27. 驗收格狀列表與響應式導覽。
  - 簡易說明：檢查桌面五欄、較小螢幕自動縮減欄數、搜尋與篩選，以及題目詳情返回各頁面。
- [x] 28. 將題目卡片改成第一版的正方形樣式。
  - 簡易說明：移除額外圖片與堆疊裝飾，回到第一版純文字卡片；卡片外框改為正方形，保留五欄與卡片下方標籤。
- [x] 29. 將題目卡片調整為 2:1 橫向長方形。
  - 簡易說明：保留目前卡片寬度，將高度縮為寬度的一半，並縮短內部間距以維持題號、狀態、標題與難度的可讀性。
...
- [x] 核可後已改成無資料夾圖案的題目卡片；桌面格線固定可容納 5 張卡片，現有題庫因只有 4 題所以顯示 4 張。
- [x] 從解題頁測試前往 Home、File、Problems 與 Settings；每個按鈕都會離開題目並顯示正確頁面。
- [x] 1440 × 900 與 390 × 844 均無水平溢位，較窄畫面會自動減少欄數。
- [x] 題目卡片已移除圖片與額外堆疊裝飾，回復第一版資訊配置並改成正方形外框。
- [x] 依最新確認將卡片由正方形改為 2:1 橫向長方形，高度縮為寬度的一半，五欄與下方標籤維持不變。
